...
-- =============================================================================
-- A página Leads sabia fatiar a base por comportamento (esperando resposta,
-- sem giro, sem dono) mas não por etiqueta — e etiqueta é justamente como a
-- mesa separa campanha ("Plano de 2026", "Comunidade Whatsapp") e passo da
-- ativação. Sem esse corte, medir campanha só dava no relatório, nunca na
-- fila de trabalho.
--
-- A view ganha as etiquetas de cada lead em dois arrays. Poderia ser uma
-- junção com lead_tags, mas aí cada lead viraria N linhas e toda contagem da
-- tela sairia inflada; e resolver por fora, buscando os ids da etiqueta antes,
-- estoura a URL do PostgREST em etiqueta grande.
--
-- v_tags_uso serve o seletor: nome, cor e quantos leads cada etiqueta tem.
--
-- De carona vem ultimo_disparo_em, que o disparo em massa precisa para não
-- repetir template na mesma pessoa (a explicação está na coluna).
--
-- Script reexecutável.
-- =============================================================================

-- lead_tags tem PK (lead_id, tag_id): filtrar só por tag_id varria a tabela.
...
-- Quantos leads cada etiqueta tem hoje. Inclui etiqueta zerada (left join):
-- ver "0" ao lado de uma campanha recém-criada é informação, não ruído.
create or replace view v_tags_uso as
select
  t.id,
  t.nome,
  t.cor,
  t.ativo,
  count(lt.lead_id) as leads
from tags t
left join lead_tags lt on lt.tag_id = t.id
group by t.id, t.nome, t.cor, t.ativo;

alter view v_tags_uso set (security_invoker = true);
