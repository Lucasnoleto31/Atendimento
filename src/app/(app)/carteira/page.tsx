...
function urlCarteira(filtro: string, status: string) {
  const p = new URLSearchParams();
  if (filtro === "minha") p.set("f", "minha");
  if (status !== "todos") p.set("s", status);
  const q = p.toString();
...
export default async function CarteiraPage({
  ...
  // Ordenação no banco garante que o corte de 2000 fica com os mais parados.
  ...
  const linhas = ((data ?? []) as LinhaCarteira[]).sort((a, b) => {
    const peso =
      (PESO_STATUS[a.status] ?? 9) - (PESO_STATUS[b.status] ?? 9);
    if (peso !== 0) return peso;
    // Dentro do status, quem está parado há mais tempo primeiro.
    return (b.dias_sem_giro ?? -1) - (a.dias_sem_giro ?? -1);
  });

  const contagem = (status: string) =>
    linhas.filter((l) => l.status === status).length;
  const temReceita = linhas.some((l) => (l.receita_30d_centavos ?? 0) > 0);
  const receita30d = linhas.reduce(
    (s, l) => s + (l.receita_30d_centavos ?? 0),
    0,
  );

  // Visão do gestor: saúde por assessor.
    ...
    <div className="flex min-h-full flex-col p-2 md:p-3">
      <header className="border-b border-neutral-200 pb-2">
        ...
        <p className="mt-1 max-w-[68ch] text-sm text-neutral-600">
          Os clientes da assessoria por saúde de giro — quem está esfriando
          aparece primeiro, para o resgate acontecer antes do churn.
        </p>
      </header>
      ...
      {error ? (
        ...
        <>
          ...
          <nav aria-label="Filtros da carteira" className="mt-2 flex flex-wrap gap-1">
            {ehGestor ? (
              <>
                {(
                  [
                  ...
                  <Link
                    key={chave}
                    href={urlCarteira(chave, statusFiltro)}
            ...
            {FILTROS_STATUS.map((s) => (
              <Link
                key={s.chave}
                href={urlCarteira(filtro, s.chave)}
          ...
          {linhas.length === 0 ? (
            <div className="mt-3 max-w-[68ch] rounded-lg border border-neutral-200 bg-neutral-0 p-3 shadow-sm">
              <h2 className="text-h3 text-neutral-900">Carteira vazia</h2>
              <p className="mt-1 text-sm text-neutral-600">
                Os clientes entram aqui pela importação diária de clientes e
                lotes, na Administração.
              </p>
            </div>
          ) : (
            <div className="mt-2 overflow-x-auto rounded-lg border border-neutral-200 bg-neutral-0 shadow-sm">
                  ...
                  {linhas.map((linha) => {
                    ...
                    const variacao =
                      ...
                      linha.lotes_30d_anterior > 0
                        ...
                        : null;
                    return (
                      <tr key={linha.customer_id} className="h-[48px] hover:bg-neutral-50">
                        <td className="px-2">
                        ...
                        <td className="px-2">
                          {linha.lead_id ? (
                            ...
                            <span className="text-xs text-neutral-400">sem lead</span>
                          )}
                        </td>
                      </tr>
                    );
